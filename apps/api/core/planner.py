...
- Wikipedia (ALWAYS use the extract API, never the raw wiki page) → https://en.wikipedia.org/w/api.php?action=query&prop=extracts&titles=PAGE_TITLE&format=json&explaintext=true&exintro=true
  (replace spaces with underscores in the page title; returns the introduction section as plain text in JSON)
...
- "Tell me about TU Wien from Wikipedia" → fetch_url with https://en.wikipedia.org/w/api.php?action=query&prop=extracts&titles=TU_Wien&format=json&explaintext=true&exintro=true
